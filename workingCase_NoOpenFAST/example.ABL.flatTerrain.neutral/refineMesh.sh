...
#reconstructPar -time '200' > log.reconstructPar

echo "      *selecting cells to refine..."
topoSet -dict system/topoSetDict.local.1 > log.topoSet.local.1 2>&1
echo "      *refining cells..."
refineHexMesh local -overwrite > log.refineHexMesh.local.1 2>&1
echo "      *selecting cells to refine..."
topoSet -dict system/topoSetDict.local.2 > log.topoSet.local.2 2>&1
echo "      *refining cells..."
refineHexMesh local -overwrite > log.refineHexMesh.local.2 2>&1
...
checkMesh -parallel > log.checkMesh.3 2>&1
