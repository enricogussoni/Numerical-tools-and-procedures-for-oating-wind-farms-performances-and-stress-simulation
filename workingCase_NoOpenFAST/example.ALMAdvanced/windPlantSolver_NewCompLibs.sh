...
#mpirun --map-by core --bind-to core renumberMesh -overwrite -parallel &> log.renumberMesh

#reconstructPar -time '91000' > log.reconstructPar
#mpirun --map-by core --bind-to core  sample -parallel &> log.sample
arrayDir=270;

#mpirun --map-by core --bind-to core  windPlantSolver.ALMAdvancedOpenFAST -parallel &> log.windPlantSolver.newLibs
mpirun --map-by core --bind-to core  windPlantSolver.ALMAdvanced -parallel &> log.windPlantSolver.NoFAST

finishOK=$?

echo " Application finished as:" ${finishOK}
